...
The Data Science team have also requested to use GPUs in their Data Science Workbenches e.g. for example - when they use a Pytorch, CUDA or other stack image that can directly access an accelerator.
...
. We can see in this case that we have a https://aws.amazon.com/ec2/instance-types/g6e[**g6e.2xlarge**] instance. We can then check how many GPUs we are able to allocate:
+
[source,bash,options="wrap",role="execute"]
----
...
. Now apply the https://github.com/redhat-ai-services/etx-agentic-ai/tree/main/infra/applications/gpu[GPU Cluster Policy and ConfigMap] objects that setup https://docs.nvidia.com/datacenter/cloud-native/gpu-operator/latest/gpu-sharing.html[Time Slicing] - a method to share nvidia gpus.
+
[source,bash,options="wrap"]
----
