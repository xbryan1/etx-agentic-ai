...
. Check the gathered api tokens are set as env vars in the secrets file.
+
.Add API Tokens
image::add-apikeys-secrets.png[Add API Tokens, 300]
+
TIP: you can create a secrets file from scratch by copying the example file provided. Only do this if you are using your own MaaS API keys.
+
[source,bash,options="wrap",role="execute"]
----
